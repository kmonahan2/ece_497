...
public class Core {
    ...
    public boolean AUSF (UE ue) {
        ...
        policy = PCF(ue);
        int capacity = policy.get("capacity");
        int threshold = policy.get("threshold");
        
        for (int i = 0; i < known_ue.getRowCount(); i++) {
            if (ueId == Integer.parseInt(known_ue.getValueAt(i,0).toString())) {
                authResult = true;
                ue.setKnown(true);
                priority = ue.getPriority();
                cb = ue.isCallbar();
                premptCap = ue.isPremtCapable();
            }
        }
            ...
            if (count >= capacity) {
                ...
                // if the UE is capable of premption,
                    ...
                    for (int p = threshold; p < priority && flag == false; p++) {
                        ...
                        for (int k=1; k <= registered_ue.getRowCount(); k++) {
                            // if a registered ue is prempt vul,
                            if(Boolean.parseBoolean(registered_ue.getValueAt(k,5).toString())){
                                // check if priority is less than threshold
                                if (Integer.parseInt(registered_ue.getValueAt(k,2).toString())<=p) {
    ...
    public synchronized boolean SMF(UE ue) {
        ...
        if (type.equals("D")) {
            if (data_width >= BANDALLO) {
                ...
                if (preCap) {
                    for (int p = 0; p < data_bd.getRowCount(); p++) {
    ...
    public synchronized HashMap<String, Integer> PCF(UE ue) {
        ...
        int bandwidth = B_ALLO;
        ret.put("band", bandwidth);
